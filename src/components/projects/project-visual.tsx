...
export function ProjectVisual({ project }: { project: Project }) {
  return (
    <div className="overflow-hidden rounded-xl border border-[var(--line)] bg-[var(--surface-strong)] p-5 sm:p-8" aria-hidden="true">
        ...
        {project.architecture.map((item, index) => (
          <div key={item} className="relative rounded-lg border border-[var(--line)] bg-[var(--background)] p-4">
            ...
            {index < project.architecture.length - 1 ? (
              <span className="absolute -bottom-3 left-1/2 z-10 grid size-6 -translate-x-1/2 place-items-center rounded-full bg-[var(--cobalt)] text-xs text-white md:-right-3 md:bottom-auto md:left-auto md:top-1/2 md:-translate-y-1/2 md:translate-x-0">
